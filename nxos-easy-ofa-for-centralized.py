...
supported = [
    "Nexus 3016", "Nexus 3064", "Nexus 3048", "Nexus 3132", "Nexus 3172T",
    "Nexus 3172", "Nexus5548", "Nexus5596", "Nexus 6001", "Nexus 6004",
    "Nexus 3548", "Nexus 3524"
    ]
...
def max_flows(platform):
    ...
    if (platform == "Nexus 6001" or platform == "Nexus 6004"):
        ...
        cli("end")
    elif (platform == "Nexus 3016" or platform == "Nexus 3064" or
            platform == "Nexus 3048" or platform == "Nexus 3132" or
            platform == "Nexus 3172" or platform == "Nexus 3172T"):
        cli("conf t")
...
def main():
    ...
    parser.add_argument("-p", "--package", required = True,
                        help = "OpenFlow Plug-in OVA package to install")
    parser.add_argument("-i", "--interfaces", required = True,
                        help = "Interfaces (separated by comma) to use for OpenFlow.")
    parser.add_argument("-c", "--controller", required = True,
                        help = "The OpenFlow controller IP address.")
    parser.add_argument("-m", "--maxflows", help =
                        "Tune switch TCAM to support maximum number of flows",
                        required = False, action = "store_true")
    parser.add_argument("-V", "--vrf", help="VRF", default="default",
                        required = False)
    ...
    vrf = args.vrf
    print 'OpenFlow Package:', package
    print 'Target Interfaces:', interfaces
    print 'Controller IP:', controller
    print 'VRF:', vrf
    ...
    cli("conf t")
    if platform == "Nexus 3548" or platform == "Nexus 3524":
        cli("hardware profile forwarding-mode openflow-hybrid")
    else:
        cli("hardware profile openflow")
    cli("end")

    print "Installing Cisco Plug-in for OpenFlow ..."
    cli("virtual-service install name ofa package bootflash:" + package)
    while True:
    ...
    if platform == "Nexus 3548" or platform == "Nexus 3524":
        ...
        cli("pipeline 201")
    cli("controller ipv4 " + controller + " port 6653 vrf " + vrf +
        " security none")
